...
- `scaling` — throughput(2 workers) ≥ 1.5 × throughput(1); throughput(4) ≥ 2.5 × throughput(1).
...
- **Scenario runtime knobs** (final values): baseline/hot-tenant 30 s; scaling 3×15 s at 2500 rps offered, concurrency 200, workers 50m; worker-kill 45 s, kill at t=15; coordinator-kill 60 s, kill t=10, restore t=25. Worker-kill turned out *cleaner* than designed — zero client-visible errors — because Go's HTTP client transparently retries idempotent requests whose reused connection died with the pod.
